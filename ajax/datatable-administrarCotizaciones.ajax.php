  		...
  		$datosJson = '
      {
        ...
        "recordsFiltered": '.intval($totalRenglones).',
		  ...
		  for($i = 0; $i < count($cotizaciones); $i++){
                    ...
                    //Ticket
                    $botones .= "<button class='btn btn-info btnImprimirTicketC'  required data-toggle='tooltip' data-placement='top' title='Imprimir Ticket' ";
                    $botones.="codigoVenta='".$cotizaciones[$i]["UUID"]."''>";
                    $botones.=" <i class='fa fa-print'></i>";
                    $botones.=" </button>";
                    //COTIZACIONES PDF
                    $botones.="  <button class='btn btn-danger btnImprimirCotizacion' required data-toggle='tooltip' data-placement='top' title='Imprimir Cotizaciones PDF'";
                    $botones.="    codigoVenta='".$cotizaciones[$i]["UUID"]."'>";
